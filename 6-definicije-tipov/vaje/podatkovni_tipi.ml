...
let dollar_to_euro (Dollar d) = Euro (d *. 0.8)
let euro_to_dollar (Euro e) = Dollar (e *. 1.3)
...
(*ni treba različnih dajat x, xs pa y, ys*)
(*
let rec intbool_map f_int f_bool = function
       | Int(x, xs) -> Int(f_int x, intbool_map f_int f_bool xs)
...
let rec intbool_separate ib_list =
       let rec intbool_separate' iacc bacc = function
              | Nil -> (iacc, bacc)
              | Int (x, xs) -> intbool_separate' (x :: iacc) bacc xs
...
(*
ČAROVNIK VEŠ DA JE PODAN Z name & status ZATO LAHKO NAPIŠEŠ KAR
       {name; status} :: wizards
NAMESTO IF LAHKO UPORABLJAŠ MATCH POVSOD!
*)
(*----------------------------------------------------------------------------*]
