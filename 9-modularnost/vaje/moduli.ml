...
module Nat_int : NAT = struct
  ...
  let one = 1
  let addition = ( + )
  let substraction x y = max 0 (x - y)
  let multiplication = ( * )
  let to_int i = i
  let of_int n = n
...
module Nat_peano : NAT = struct
  ...
  let rec eq x y = 
    ...
    | Nic, Nic -> true
    | Nic, _  | _, Nic -> false
    | Naslednik xx, Naslednik yy -> eq xx yy

  let zero = Nic
  let one = Naslednik Nic

  let rec addition x = function
    | Nic -> x
    | Naslednik yy -> Naslednik (addition x yy)

  let rec substraction x y =
    match x, y with
    | x, Nic -> x
    | Nic, _ -> Nic
    | Naslednik xx, Naslednik yy -> substraction xx yy

  let rec multiplication x = function
    | Nic -> Nic
    | Naslednik yy -> addition (multiplication x yy) x
  ...
  let rec to_int = function
    | Nic -> 0
    | Naslednik x -> 1 + to_int x
  
  let rec of_int = function
    | i when i <= 0 -> Nic
    | x -> Naslednik (of_int (x-1))
...
let sum_nat_100 (module Nat : NAT) =
  ...
  let rec sum current acc =
    if Nat.eq current hundred then
      ...
      sum (Nat.addition current Nat.one) (Nat.addition acc current)
  in
...
module Polar : COMPLEX = struct
  ...
  let add x y = ()
  let mul x y = 
    let magn = x.magn *. y.magn  in
    let arg = x.arg +. y.magn  in
    {magn; arg}
